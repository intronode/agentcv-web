...
export default async function TeamFileNewPage({ params }: PageProps) {
  const { slug } = await params;

  const session = await auth();
  if (!session?.user?.id) {
    redirect(`/auth/signin?callbackUrl=/teams/${slug}/files/new`);
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <FileUploadForm subjectType="team" subjectSlug={slug} returnUrl={`/teams/${slug}`} />
    </div>
  );
}
